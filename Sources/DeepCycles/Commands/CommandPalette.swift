...
struct CommandPalette: View {
    ...
    private var commands: [AppCommand] {
        CommandCatalog.commands(store: store, ui: ui, engine: engine, openSettings: { openSettings() })
    ...
    private var matches: [AppCommand] {
        ...
        return all.filter { FuzzyMatch.matches(q, in: $0.title) || $0.group.lowercased().hasPrefix(q) }
    }
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            for delay in [0.03, 0.12, 0.3] {
                DispatchQueue.main.asyncAfter(deadline: .now() + delay) { if ui.showPalette, !focused { focused = true } }
            }
            ...
            keyMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { e in
                ...
                case 53:  ui.showPalette = false; return nil                                // esc
                default: break
                }
