...
struct SystemsView: View {
    ...
    enum Page: Hashable {
        case root, doc(CoreDoc.Kind), week, disciplines, sessions

        var key: String {
            switch self {
            case .root: return "root"; case .week: return "week"; case .disciplines: return "disciplines"; case .sessions: return "sessions"
            case .doc(let k): return k.rawValue
            }
        }
        static func from(_ key: String) -> Page {
            if let k = CoreDoc.Kind(rawValue: key) { return .doc(k) }
            switch key { case "root": return .root; case "disciplines": return .disciplines; case "sessions": return .sessions; default: return .week }
        }
    }
    ...
    var body: some View {
        ...
        .onAppear { page = Page.from(store.systemsPage) }
        .onChange(of: store.systemsPage) { page = Page.from($0) }
        .onChange(of: page) { store.systemsPage = $0.key }
    }
        ...
        VStack(alignment: .leading, spacing: 4) {
            ...
            group("Core documents") {
                ...
                ForEach(CoreDoc.Kind.allCases, id: \.self) { k in
                    if let d = store.system.docs.first(where: { $0.kind == k }) {
    ...
    private var rootPage: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            editor(rootBinding, placeholder: rootTemplate, minHeight: 420)
    ...
    private var rootBinding: Binding<String> {
        Binding(get: { store.system.root }, set: { store.system.root = $0 })
    }
        ...
        VStack(alignment: .leading, spacing: 18) {
            HStack(alignment: .firstTextBaseline) {
                ...
                Button("Copy last week") {
                    ...
                    if let p = store.system.weeks[prev] { store.thisWeek = p }
            ...
            HStack(spacing: 10) {
                Button("Read Values") { page = .doc(.values) }.buttonStyle(QuietButtonStyle())
                Button("Read Career plan") { page = .doc(.career) }.buttonStyle(QuietButtonStyle())
                Button("Read Personal plan") { page = .doc(.personal) }.buttonStyle(QuietButtonStyle())
                Spacer()
                Button("Time-block today") { store.tab = 0 }.buttonStyle(InkButtonStyle())
            }
        }
    }
    ...
    private func docPage(_ kind: CoreDoc.Kind) -> some View {
        let doc = store.system.docs.first { $0.kind == kind }
        return VStack(alignment: .leading, spacing: 14) {
            ...
            if kind == .tasks {
                ...
                editor(store.doc(kind), placeholder: "", minHeight: 460)
            }
        }
    }
        ...
        return Grid(horizontalSpacing: 6, verticalSpacing: 6) {
            ...
            ForEach(store.system.disciplines) { disc in
                GridRow {
                    ...
                    ForEach(days, id: \.self) { d in
                        let v = store.plan(for: d).disciplineLog[disc.id.uuidString] ?? ""
                        let hit: Bool = {
                            guard !v.isEmpty else { return false }
                            if disc.isNumber { return (Double(v) ?? 0) >= (Double(disc.target) ?? 0) && (Double(v) ?? 0) > 0 }
                            return v == "1"
                        }()
        ...
        return VStack(alignment: .leading, spacing: Space.l) {
            ...
            ForEach(Array(log.enumerated()), id: \.offset) { _, e in
                SessionLogRow(date: e.date, block: e.block, session: e.session) {
                    let key = Store.key(for: e.date)
                    store.plans[key]?.sessions.removeAll { $0.id == e.session.id }
                }
            }
        }
    }
                ...
                if open {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        ForEach(Array(session.cycles.enumerated()), id: \.offset) { i, c in
                            if !c.goal.isEmpty {
                                note("Cycle \(i + 1)", c.goal + (c.completed.isEmpty ? "" : " — \(c.completed)") + (c.improvements.isEmpty ? "" : " · improve: \(c.improvements)"))
                            }
                        }
                    }
