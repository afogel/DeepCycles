...
/// The end-of-day sheet: metrics, disciplines, the shutdown checklist and the "complete" switch.
@MainActor
struct ShutdownView: View {
    ...
    private var loggedDeepMinutes: Int { store.today.deepMinutesLogged }
    private var cyclesHit: (Int, Int) {
        let answered = store.today.sessions.flatMap { $0.cycles }.filter { $0.completed != nil }
        return (answered.filter { $0.completed == .yes }.count, answered.count)
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                ...
                Spacer()
                Button("Close  ⎋") { ui.showShutdown = false }.buttonStyle(QuietButtonStyle()).keyboardShortcut(.cancelAction)
            }
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    HStack(spacing: 12) {
                        ...
                        stat("deep work, planned", hours(store.today.deepMinutesPlanned), Theme.deep.opacity(0.55))
                    ...
                    if !store.system.disciplines.isEmpty {
                        VStack(alignment: .leading, spacing: 10) {
                            HStack {
                                ...
                                Spacer()
                                Button("Edit list") { ui.showShutdown = false; ui.showSystems(.disciplines) }.buttonStyle(QuietButtonStyle())
                            }
                        ...
                        HStack {
                            ...
                            Button("Fill DW from cycles") {
                                if let i = store.today.metrics.firstIndex(where: { $0.name.hasPrefix("DW") }) {
                                    store.today.metrics[i].value = deepHours
                                } else {
                                    store.today.metrics.append(Metric(name: "DW (deep work hrs)", value: deepHours))
                                }
                            }.buttonStyle(QuietButtonStyle())
                            Spacer()
                        }
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        let logged = store.system.disciplines.allSatisfy { !store.today.discipline($0.id).isEmpty }
                    ...
                    if store.today.shutdownComplete {
                        Text("Done for the day. When work anxiety shows up tonight: you checked the box. Close the planner.")
                            .foregroundColor(Theme.inkFaint)
                        Button("Close the planner") { ui.showShutdown = false }.buttonStyle(InkButtonStyle(fill: Theme.breakC))
                    }
                }
                .padding(28)
        ...
        .onExitCommand { ui.showShutdown = false }
    }
    ...
    private var deepHours: String { String(format: "%.1f", Double(loggedDeepMinutes) / 60) }
    ...
    private func disciplineRow(_ d: Discipline) -> some View {
        let value = Binding<String>(
            get: { store.today.discipline(d.id) },
            set: { store.today.logDiscipline(d.id, $0) }
        )
        return HStack(spacing: 12) {
            ...
            if d.isNumber {
                if d.code.uppercased() == "DW" {
                    Button("from cycles") { value.wrappedValue = deepHours }
                        .buttonStyle(QuietButtonStyle())
                }
                ...
                Toggle("", isOn: Binding(get: { store.today.disciplineDone(d.id) }, set: { store.today.setDisciplineDone(d.id, $0) }))
